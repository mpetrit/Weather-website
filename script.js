...
async function displayWeather() {
  try {
    const response = await fetch(
      `https://api.openweathermap.org/data/2.5/weather?q=${cityName}&appid=${apiKey}&units=${weatherUnit}`
    );
    const data = await response.json();

    console.log(data);
    //place country in the input field, check if u run it again
    inputField.value.includes(regionNamesInEnglish.of(data.sys.country)) == 0
      ? (inputField.value += `, ${regionNamesInEnglish.of(data.sys.country)}`)
      : "";
    ...
    temp.textContent =
      Math.round(data.main.temp) + (weatherUnit == "metric" ? " ᵒC" : " ᵒF");
    ...
    //all-info
    minTemp.textContent = `Lows: ${Math.round(data.main.temp_min)} ${
      weatherUnit == "metric" ? " ᵒC" : " ᵒF"
    }`;
    feelsLike.textContent = `Feels like: ${Math.round(data.main.feels_like)} ${
      weatherUnit == "metric" ? " ᵒC" : " ᵒF"
    }`;
    maxTemp.textContent = `Highs: ${Math.round(data.main.temp_max)} ${
      weatherUnit == "metric" ? " ᵒC" : " ᵒF"
    }`;

    humidity.textContent = `${data.main.humidity} %`;
    windSpeed.textContent = `${data.wind.speed} ${
      weatherUnit == "metric" ? "m/s" : "mph"
    }`;
    ...
    console.error(error);
    inputField.value = "";
    shallowResults.innerHTML = "<h3>Something went wrong! Try again.</h3>";
    minTemp.textContent = "-";
